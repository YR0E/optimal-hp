...
if theme is not None and theme["base"] == "dark":
    ...
    theme_session = (
        "streamlit"  # to handle some streamlit issue with rendering plotly template
    )
else:
...
def tab_eps_total_plane():
    ...
    with col_control:
        init_eps_total = init_slider(
            r"$\varepsilon_{total}:$", "e_t_e", 0.4, 4.0, 0.1, fmt="%.1f"
        )
        ...
        init_q = init_slider(
            ...
            0.1,
            fmt="%.1f",
            help=rf"$q_{{0}} \times 10^{{-{POWER_OF_10:.0f}}}$",
        )
        init_t_s = init_slider("$t_{s}:$", "t_s_e", 0.8, 1.0, 0.01)
        init_I = init_slider("$I:$", "I_e", 1.0, 2.0, 0.01)

        s_value = (
            ((init_I - 1) * init_q / MULTIPLIER)
            / (
                init_t_s
                - (2 * init_q / (MULTIPLIER * (init_c_g + init_c_p)))
                * (8 / init_eps_total - 1)
            )
            * MULTIPLIER
        )
        init_s = init_slider(
            ...
            0.01,
            value=s_value,
            help=rf"$s \times 10^{{-{POWER_OF_10:.0f}}},\quad s=f(I, q_{{0}}, t_s, \varepsilon, c) $",
        )

        st.button(
            "Reset", on_click=lambda: reset_sliders(DEFAULT_VALUES_EPS), key="btn_e"
        )
    ...
    df = pd.DataFrame(
        {
            ...
            f"min(w) [e-{POWER_OF_10:.0f}]": np.array(
                [results["r"].fun, results["ir"].fun, results["ep"].fun]
            )
            * MULTIPLIER,
        },
...
def tab_c_total_plane():
    ...
    with col_control:
        ...
        s_value = (
            ...
            / (
                init_t_s
                - (2 * init_q / (MULTIPLIER * init_c_total))
                * (8 / (init_e_g + init_e_p) - 1)
            )
            * MULTIPLIER
        )
        init_s = init_slider(
        ...
        st.button(
            "Reset", on_click=lambda: reset_sliders(DEFAULT_VALUES_C), key="btn_c"
        )
...
with tab_eps_total:
    st.write(
        r"Curzon-Ahlborn model: imposed $q_0 - \varepsilon_{total}$: find minimum power consumption $\min(w)$"
    )
    st.write("")
    tab_eps_total_plane()
...
DEFAULT_SA_SLIDERS = {
    ...
    "s": {"e_t": 3.2, "c_t": 0.8, "q0": 50.0, "t_s": 0.9, "s": (0.0, 25.0)},
}
...
def settings_popover(var, defaults):
    ...
    with st.popover(
        ...
        icon=":material/tune:",
        help="Set sensitivity analysis parameters",
    ):
        st.write(
            f"""
        ...
        st.button(
            "Reset",
            on_click=lambda: reset_sliders(defaults),
            key=f"reset_set_{var}",
        ...
        left, _, right = st.columns(columns_size, vertical_alignment="top")
        opt_method = left.selectbox(
            "Opt. method:", methods_list, disabled=True, key=f"{var}_opt_method"
        )
        tolerance = right.slider(
            "Tolerance, $10^{x}$:", min_value=-24, max_value=-6, key=f"{var}_tol"
        )
        ...
        warm_start = st.toggle(
            "Vectorize with warm starting*",
            key=f"{var}_warm_start",
...
def calculate_gradient(dfs, smooth=False):
    columns = [
        col for col in dfs[0].columns if not (col.endswith("ev") or col.endswith("cd"))
    ]
...
def tab_e_total_sa():
    ...
    with col_info:
        # settings
        step_size, var_range, *opt_settings = settings_popover(
            "e", DEFAULT_SETTING_VALUES
        )
        (
            opt_method,
            tolerance,
        ...
        txt = "(warm starting*)" if warm_start else ""
    ...
    with col_control:
        ...
        st.button(
            "Reset",
            on_click=lambda: reset_sliders(DEFAULT_SA_SLIDERS),
            key="btn_sae",
    ...
    initial_params = {
        ...
        "ep": [
            ...
            init_I,
            MULTIPLIER,
        ],  # s=f(I,q,...) in find_minimum_vectorized
    }
...
def sensitivity_analysis():
    ...
    st.info(
        r"Choose a variable/parameter to analyze its impact on the minimum power consumption $min(w)$"
    )
