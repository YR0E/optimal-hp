...
with tab_eps_total:
    ...
    # with col_control.form("sliders", border=False):
        ...
        col1, col2 = st.columns((0.15, 0.85))
        col1.markdown('$s:$', help=fr'$s \times 10^{{-{POWER_OF_10:.0f}}}$')
    ...
    df = pd.DataFrame({
        ...
        f'min(w) [e-{POWER_OF_10:.0f}]': np.array([optimization_result.fun, optimization_result_ir_ratio.fun, optimization_result_ep_rate.fun]) * MULTIPLIER
        }, 
...
with tab_c_total:
    ...
    # with col_control.form("sliders", border=False):
        ...
        col1.write(r'$\varepsilon_{total}:$')
        init_eps_total = col2.slider('e_tot', min_value=0.4, max_value=4.0, step=0.1,
                                    format="%.1f", label_visibility="collapsed", key="eps_total2")

        col1, col2 = st.columns((0.15, 0.85))
        col1.write('$c_{g}:$')
        init_c_g = col2.slider('c_g', min_value=0.05, max_value=0.5, step=0.01,
                            format="%.2f", label_visibility="collapsed", key="c_g2")

        col1, col2 = st.columns((0.15, 0.85))
        col1.write('$c_{p}:$')
        init_c_p = col2.slider('c_p', min_value=0.05, max_value=0.5, step=0.01,
                            format="%.2f", label_visibility="collapsed", key="c_p2")

        col1, col2 = st.columns((0.15, 0.85))
        col1.markdown('$q_{0}:$', help=fr'$q_{{0}} \times 10^{{-{POWER_OF_10:.0f}}}$')
        init_q = col2.slider('q0', min_value=1.0, max_value=50.0, step=0.1,
                            format="%.1f", label_visibility="collapsed", key="q02")

        col1, col2 = st.columns((0.15, 0.85))
        col1.write('$t_{s}:$')
        init_t_s = col2.slider('t_s', min_value=0.8, max_value=1.0, step=0.01,
                            format="%.2f", label_visibility="collapsed", key="t_s2")

        col1, col2 = st.columns((0.15, 0.85))
        col1.write('$I:$')
        init_I = col2.slider('I', min_value=1.0, max_value=2.001, step=0.01,
                            format="%.2f", label_visibility="collapsed", key="I2")

        col1, col2 = st.columns((0.15, 0.85))
        col1.markdown('$s:$', help=fr'$s \times 10^{{-{POWER_OF_10:.0f}}}$')
        init_s = col2.slider('s', min_value=0.1, max_value=30.0, step=0.01,
                            format="%.2f", label_visibility="collapsed", key="s2")
    ...
    with col_plot:
        st.plotly_chart(fig, use_container_width=True, config=config, key='c')
